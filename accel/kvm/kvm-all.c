...
//	printf("%s,and guest addr: 0x%llx,and flags is 0x%x,memorysize:0x%lx\n",__func__,mem.guest_phys_addr,mem.flags,slot->memory_size);
                                  ...
                                  bool assign, uint32_t size, bool datamatch)
{
	...
	printf("%s,and addr is 0x%llx,size:0x%x,fd:%d\n",__func__,iofd.addr,iofd.len,iofd.fd);
    if (!kvm_enabled()) {
        return -ENOSYS;
    }
...
//	printf("%s,and its name is %s,mr addr:0x%lx, size: 0x%lx,mr ram is %d,writeable is %d\n",__func__,mr->name,mr->addr,int128_get64(mr->size),mr->ram,writeable);
...
//	printf("%s,and by dhw\n",__func__);
	...
	printf("%s,and before kvm_set_irq\n",__func__);
...
static int kvm_init(MachineState *ms)
{
	...
	printf("%s,and kvm_type is %s\n",__FUNCTION__,kvm_type);
	printf("%s,and mc->kvm_type is %p\n",__FUNCTION__,mc->kvm_type);
    if (mc->kvm_type) {
...
#endif
	printf("%s,and kvm_direct_msi_allowed is %d\n",__FUNCTION__,kvm_direct_msi_allowed);
...
int kvm_cpu_exec(CPUState *cpu)
{
    ...
    do {
        ...
        case KVM_EXIT_IO:
            ...
            break;
        case KVM_EXIT_MMIO:
	//	if((run->mmio.phys_addr & 0xfeb80000) == 0xfeb80000)
	  //          printf("%s,handle_mmio,and addr:0x%llx,len:%d,is_write:%d\n",__func__,run->mmio.phys_addr,run->mmio.len,run->mmio.is_write);
            /* Called outside BQL */
