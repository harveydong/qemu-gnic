...
static inline void _nocheck__trace_colo_compare_tcp_info(const char * pkt, uint32_t seq, uint32_t ack, int res, uint32_t flag, int size)
{
    if (trace_event_get_state(TRACE_COLO_COMPARE_TCP_INFO)) {
        ...
        gettimeofday(&_now, NULL);
        qemu_log_mask(LOG_TRACE,
                      "%d@%zd.%06zd:colo_compare_tcp_info " "side: %s seq/ack= %u/%u res= %d flags= %x pkt_size: %d\n" "\n",
                      getpid(),
                      (size_t)_now.tv_sec, (size_t)_now.tv_usec
...
static inline void _nocheck__trace_colo_filter_rewriter_pkt_info(const char * func, const char * src, const char * dst, uint32_t seq, uint32_t ack, uint32_t flag)
{
    if (trace_event_get_state(TRACE_COLO_FILTER_REWRITER_PKT_INFO)) {
        ...
        gettimeofday(&_now, NULL);
        qemu_log_mask(LOG_TRACE,
                      "%d@%zd.%06zd:colo_filter_rewriter_pkt_info " "%s: src/dst: %s/%s p: seq/ack=%u/%u  flags=%x\n" "\n",
                      getpid(),
                      (size_t)_now.tv_sec, (size_t)_now.tv_usec
